#!/bin/bash
# D-BASE-S1: one 30-minute attract-mode soak session with host-side sampling.
# usage: s1_session.sh <LABEL> [HOLD_S]
set -u
...
if [ "$OPENED" != "1" ]; then
  echo "$LABEL FAILED_TO_OPEN" >> "$HERE/s1_index.txt"
  echo "[$LABEL] FAILED to open stream"; exit 1
fi

echo "[$LABEL] streaming from $(date -u +%H:%M:%SZ) for ${HOLD}s"
python3 "$HERE/s1_sampler.py" "$OUT/samples.jsonl" "$HOLD" &
SAMPLER=$!
sleep "$HOLD"
...
if [ -n "$REPORT" ]; then
  echo "$LABEL $REPORT completed" >> "$HERE/s1_index.txt"
  echo "[$LABEL] report: $REPORT"
else
  echo "$LABEL NOREPORT" >> "$HERE/s1_index.txt"
...
adb shell am start -n com.safeiot.privyhub/.MainActivity >/dev/null 2>&1
sleep 5
adb shell uiautomator dump /sdcard/s1td.xml >/dev/null 2>&1
if adb shell cat /sdcard/s1td.xml 2>/dev/null | grep -qi "NOW PLAYING"; then
