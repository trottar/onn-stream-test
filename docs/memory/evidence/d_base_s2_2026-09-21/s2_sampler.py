"""D-BASE-S2 host-side sampler: one JSON line every 30 s for a session.

D-BASE-S1's sampler, plus the VmRSS / RssAnon / RssFile / RssShmem / VmSwap
split so anonymous growth can be told from file-backed cache, and MemTotal
/ MemAvailable for context. The companion matcher already requires the
argument list to begin with "python3" (the S1 defect).
...
def proc_stat(pid):
    try:
        ...
        return {
            ...
            "rss_shmem_kb": fields.get("RssShmem"),
            "swap_kb": fields.get("VmSwap"),
            "vsize_kb": fields.get("VmSize"),
