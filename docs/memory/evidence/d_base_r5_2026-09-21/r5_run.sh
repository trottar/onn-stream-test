#!/bin/bash
# D-BASE-R5 validation: one 20-minute attract-mode session, zero input.
# The radio series comes from P4's post-session harvest, which costs the run
# nothing. Companion pid is selected by EXECUTABLE BASENAME, never by a
# command-line match (the `pgrep -f` trap, TOOLS.md).
# usage: r5_run.sh <label> <hold_seconds>
set -u
REPO=/home/privyhub/Projects/onn-stream-test
SCRATCH=/tmp/claude-1000/-home-privyhub-Projects-onn-stream-test/aeea06ae-e15e-4b0d-8cfd-6f3d4f138c4f/scratchpad/r5
LABEL="$1"; HOLD="$2"
...
echo "[$LABEL] companion $NEWPID serving (8765 was free first)"
...
for attempt in 1 2; do
  ...
  sleep 7
  adb shell uiautomator dump /sdcard/r5chk.xml >/dev/null 2>&1
  if adb shell cat /sdcard/r5chk.xml | grep -q "now_playing_preview_host"; then
...
# sample the live window a few times to prove native-stream-status works
( for i in $(seq 1 6); do
    sleep $(( HOLD / 7 ))
    curl -s localhost:8765/plugins/games/native-stream-status \
      | python3 -c "
import sys,json
d=json.load(sys.stdin); n=d.get('native_stream',d)
w=d.get('loss_per_min_recent')
print('   live window:', json.dumps(w) if w else 'None')
" 2>/dev/null
  done ) > "$SCRATCH/live_${LABEL}.txt" 2>&1 &
LIVEPID=$!
...
python3 "$SCRATCH/r5_harvest.py" "$LABEL" "$SCRATCH"
...
echo "[$LABEL] live-window samples:"; cat "$SCRATCH/live_${LABEL}.txt" 2>/dev/null
