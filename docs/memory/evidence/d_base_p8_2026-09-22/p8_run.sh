#!/bin/bash
# D-BASE-P8 -- where the ~55 ms audio arrival hole comes from.
#
# One 20-minute attract-mode session per arm, the adopted profile (no
# PRIVYHUB_ENC_* variable), the P8 diagnostic client (audio.arrival_holes):
#   A  heartbeat 2 s, socket sampler ON   (as P7 ran)
#   B  heartbeat 2 s, socket sampler OFF  (no adb to the onn during the hold)
#   C  heartbeat 10 s (PRIVYHUB_HEARTBEAT_MS=10000), sampler OFF
# Derived from d_base_p7_2026-09-22/p7_run.sh; changes: DISPLAY=:0 for the
# headless host, SAMPLER=on|off, and an adb-silence monitor over the hold
# (every 0.5 s, any adb client process other than the server is logged).
#
# usage: [PRIVYHUB_HEARTBEAT_MS=n] SAMPLER=on|off p8_run.sh <arm> <hold_s> <out_dir>
set -u
export DISPLAY=:0
SAMPLER="${SAMPLER:-on}"
...
echo "[$ARM] sampler=$SAMPLER heartbeat_ms=${PRIVYHUB_HEARTBEAT_MS:-default}"
...
for attempt in 1 2; do
  ...
  sleep 7
  adb shell uiautomator dump /sdcard/p8chk.xml >/dev/null 2>&1
  if adb shell cat /sdcard/p8chk.xml | grep -q "now_playing_preview_host"; then
...
echo "$ARM $HOLD ${NEW:-none} $T0 $T1 sampler=$SAMPLER heartbeat_ms=${PRIVYHUB_HEARTBEAT_MS:-default}" >> "$SCRATCH/index.txt"
