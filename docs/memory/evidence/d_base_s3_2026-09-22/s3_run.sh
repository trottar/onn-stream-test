#!/bin/bash
# D-BASE-S3 -- the three-hour soak on the adopted cap.
#
# The cap is now a PROFILE default, so the validating run sets NO
# environment variable at all: `env | grep PRIVYHUB_ENC` must print
# nothing. The built argv is captured from `native-stream-status` BEFORE
# the hold begins and must carry `-max_frame_size 90000` with
...
# D-BASE-S3: the Opal's own view at 20 s, read-only, ONLY if BatchMode
# succeeds in one try. No retry -- the O1 rule.
O1="$REPO/docs/memory/evidence/o1_2026-09-21"
AIR="$SCRATCH/air_${ARM}.jsonl"
if timeout 15 ssh -o BatchMode=yes -o ConnectTimeout=8 opal true 2>/dev/null; then
  echo "[$ARM] opal reachable; sampling the air at 20 s"
  rm -f "$AIR" "$AIR.stop"
  nohup "$O1/o1_opal_sample.sh" "$AIR" 20 wlan1 \
    > "$SCRATCH/airsampler_${ARM}.log" 2>&1 &
else
  echo "[$ARM] opal NOT reachable on one try; air sampling SKIPPED (no retry)"
fi
sleep 20
