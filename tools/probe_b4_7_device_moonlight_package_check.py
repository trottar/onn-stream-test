...
CLASS_ABSENT = "B4_7_DEVICE_MOONLIGHT_ABSENT_CONFIRMED"
CLASS_INSTALLED = "B4_7_DEVICE_MOONLIGHT_INSTALLED"
CLASS_NOT_VERIFIED = "B4_7_DEVICE_MOONLIGHT_STATE_NOT_VERIFIED"
SCHEMA = "privyhub_b4_7_device_moonlight_package_check_v1"

MOONLIGHT_PACKAGE = "com.limelight"
PRIVYHUB_PACKAGE = "com.safeiot.privyhub"

MDNS_RE = re.compile(
    r"^adb-.*\._adb-tls-connect\._tcp$"
)
...
def decode_local_properties_sdk(
    path: Path,
) -> str | None:
    if not path.is_file():
        return None
...
def physical_candidates(
    ...
    for value in devices:
        ...
        if (
            ...
            or NETWORK_SERIAL_RE.fullmatch(
                value
            )
        ):
            result.append(value)
...
def main() -> int:
    ...
    if args.self_test:
        assert decode_local_properties_sdk(
    ...
    if (
        adb
        and target
        and privyhub_present
        and not query_error
    ):
        classification = (
            CLASS_INSTALLED
            if moonlight_present
            else CLASS_ABSENT
        )
    else:
        classification = (
    ...
    text_path = (
        out_dir
        / "b4_7_device_moonlight_package_check.txt"
    )

    lines = [
        "PrivyHub B4.7 device Moonlight package check",
        ...
        "",
        "=== PACKAGE CHECK ===",
        f"PrivyHub package present on resolved target: {privyhub_present}",
        f"Moonlight/com.limelight installed: {moonlight_present if classification != CLASS_NOT_VERIFIED else '<unverified>'}",
        f"Package query error: {query_error or '<none>'}",
        "",
        "Next step: "
        + (
            "B4_7_DEVICE_MOONLIGHT_ABSENT_BEGIN_B5_NATIVE_ONLY_REGRESSION"
            if classification == CLASS_ABSENT
            else (
                "B4_7_DEVICE_MOONLIGHT_INSTALLED_REMOVE_PACKAGE_THEN_B5"
                if classification == CLASS_INSTALLED
                else "B4_7_DEVICE_PACKAGE_STATE_NOT_VERIFIED_INSPECT_ADB_RECOVERY"
            )
        ),
    ]
