...
TEXT_LOG = Path(
    "logs/streaming/c3_fixed_5000_characterization.txt"
)
JSON_LOG = Path(
    "logs/streaming/c3_fixed_5000_characterization.json"
)
STATE_PATH = Path(
    "logs/streaming/c3_fixed_5000_characterization_state.json"
)
DECODER_ROOT = Path(
...
def _write(
    *,
    ...
    lines = [
        "PrivyHub C3 fixed 5000 kbps characterization",
        f"Result: {result}",
        "",
        "Privacy:",
...
def _start() -> int:
    ...
    if problems:
        _write(
            result="C3_FIXED_5000_START_BLOCKED",
            details=details,
            measurements=measurements,
            problems=problems,
            payload={},
    ...
    cycle = _request_json(
        "/plugins/games/c3-fixed-bitrate-5000-cycle",
        method="POST",
        timeout=12.0,
    )
    ...
    if (
        ...
        problems.append(
            "5000 kbps actuator cycle did not return expected schema"
        )
    ...
    if int(
        ...
        problems.append(
            "native status does not report 5000 kbps after cycle"
        )
    ...
    if int(
        ...
        problems.append(
            "native source bitrate does not report 5000 kbps"
        )
    ...
    if len(fresh_samples) < 3:
        problems.append(
            "fewer than three fresh post-5000 telemetry intervals captured"
        )
    ...
    state = {
        "schema": "privyhub_c3_fixed_5000_state_v1",
        "target_bitrate_kbps": TARGET_KBPS,
    ...
    details.extend(
        [
            "5000 kbps diagnostic candidate activated",
    ...
    _write(
        result=(
            "C3_FIXED_5000_RUNTIME_SAMPLE_CAPTURED"
            if not problems
            else "C3_FIXED_5000_RUNTIME_SAMPLE_CAPTURED_WITH_FINDINGS"
        ),
        details=details,
        measurements=measurements,
        problems=problems,
...
def _finalize() -> int:
    if not STATE_PATH.is_file():
        _write(
            result="C3_FIXED_5000_FINALIZE_BLOCKED",
            details=[],
            measurements={},
            problems=[
                "5000 characterization state is missing"
            ],
            payload={},
        )
        return 2
    ...
    if not candidates:
        _write(
            result="C3_FIXED_5000_FINALIZE_NEEDS_SESSION_END",
            details=[
    ...
    measurements = {
        ...
        "audio_stale_drops": audio.get(
            "stale_drops"
        ),
        "audio_max_queue_depth": audio.get(
            "max_queue_depth"
        ),
        "audio_concealed_loss_packets": audio.get(
            "concealed_loss_packets"
        ),
        "audio_concealed_underruns": audio.get(
            "concealed_underruns"
        ),
        "audio_prolonged_starvation_events": audio.get(
            "prolonged_starvation_events"
        ),
        "audio_smooth_latency_trims": audio.get(
            "smooth_latency_trims"
        ),
        "audio_crossfaded_packets": audio.get(
            "crossfaded_packets"
        ),
        "audio_max_queue_residence_ms": audio.get(
            "max_queue_residence_ms"
        ),
        "audio_avg_queue_residence_ms": audio.get(
    ...
    details = [
        "5000 kbps post-cycle telemetry and decoder-session evidence captured",
        ...
        "known audio burst/gap counters are reported but do not classify bitrate acceptance",
    ]
    ...
    _write(
        result=(
            "C3_FIXED_5000_EVIDENCE_CAPTURED"
            if not problems
            else "C3_FIXED_5000_EVIDENCE_CAPTURED_WITH_FINDINGS"
        ),
        details=details,
        measurements=measurements,
        problems=problems,
