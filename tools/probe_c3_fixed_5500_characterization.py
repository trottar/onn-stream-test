...
TEXT_LOG = Path(
    "logs/streaming/c3_fixed_5500_characterization.txt"
)
JSON_LOG = Path(
    "logs/streaming/c3_fixed_5500_characterization.json"
)
STATE_PATH = Path(
    "logs/streaming/c3_fixed_5500_characterization_state.json"
)
DECODER_ROOT = Path(
...
def _write(
    *,
    ...
    lines = [
        "PrivyHub C3 fixed 5500 kbps characterization",
        f"Result: {result}",
        "",
        "Privacy:",
...
def _start() -> int:
    ...
    if problems:
        _write(
            result="C3_FIXED_5500_START_BLOCKED",
            details=details,
            measurements=measurements,
            problems=problems,
            payload={},
    ...
    cycle = _request_json(
        "/plugins/games/c3-fixed-bitrate-5500-cycle",
        method="POST",
        timeout=12.0,
    )
    ...
    if (
        ...
        problems.append(
            "5500 kbps actuator cycle did not return expected schema"
        )
    ...
    if int(
        ...
        problems.append(
            "native status does not report 5500 kbps after cycle"
        )
    ...
    if int(
        ...
        problems.append(
            "native source bitrate does not report 5500 kbps"
        )
    ...
    if len(fresh_samples) < 3:
        problems.append(
            "fewer than three fresh post-5500 telemetry intervals captured"
        )
    ...
    state = {
        "schema": "privyhub_c3_fixed_5500_state_v1",
        "target_bitrate_kbps": TARGET_KBPS,
    ...
    details.extend(
        [
            "5500 kbps diagnostic candidate activated",
    ...
    _write(
        result=(
            "C3_FIXED_5500_RUNTIME_SAMPLE_CAPTURED"
            if not problems
            else "C3_FIXED_5500_RUNTIME_SAMPLE_CAPTURED_WITH_FINDINGS"
        ),
        details=details,
        measurements=measurements,
        problems=problems,
...
def _finalize() -> int:
    if not STATE_PATH.is_file():
        _write(
            result="C3_FIXED_5500_FINALIZE_BLOCKED",
            details=[],
            measurements={},
            problems=[
                "5500 characterization state is missing"
            ],
            payload={},
        )
        return 2
    ...
    if not candidates:
        _write(
            result="C3_FIXED_5500_FINALIZE_NEEDS_SESSION_END",
            details=[
    ...
    details = [
        "5500 kbps post-cycle telemetry and decoder-session evidence captured",
    ...
    _write(
        result=(
            "C3_FIXED_5500_EVIDENCE_CAPTURED"
            if not problems
            else "C3_FIXED_5500_EVIDENCE_CAPTURED_WITH_FINDINGS"
        ),
        details=details,
        measurements=measurements,
        problems=problems,
